...
def caculate_ece_calibrate(
    ...
    for index, domain_name in enumerate(domain_names):
        ...
        if type_calibrate == "in_domain_old":
...
def report_ece_not_calibrate(config_params, type_eces, path_save, kfold, domain_names):
...
def report_ece_calibrate_temperature(
    ...
    path_save, path_exp, kfold, domain_names, 
    type_calibrate
):
    for type_ece in type_eces:
